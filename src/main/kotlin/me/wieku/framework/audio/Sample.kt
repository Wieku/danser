...
class Sample(filename: String) {
	...
	fun play() {
		...
		BASS_ChannelSetAttribute(channel.asInt(), BASS_ATTRIB_VOL, /*settings.Audio.GeneralVolume*settings.Audio.SampleVolume*/0.5f)
	...
	fun playRV(volume: Float) {
		val channel = BASS_SampleGetChannel(mainChannel, false)
		BASS_ChannelSetAttribute(channel.asInt(), BASS_ATTRIB_VOL,/*settings.Audio.GeneralVolume*settings.Audio.SampleVolume**/volume)
		BASS_ChannelPlay(channel.asInt(), true)
	}
}