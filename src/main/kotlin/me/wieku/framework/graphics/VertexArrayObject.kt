...
class VertexArrayObject(private var maxVertices: Int, private val attributes: Array<VertexAttribute>) {
	...
	fun bindToShader(shader: Shader) {
		...
		attributes.forEach {
			...
			glVertexAttribPointer(location, it.attributeType.size / 4, if (it.attributeType == VertexAttributeType.GlInt) GL_INT else GL_FLOAT, false, attributes.vertexSize(), offset.toLong())
